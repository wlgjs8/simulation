...
OUT = Path(os.environ.get("FIT_OUT", "/home/plaif/fit5"))
GPUS = ["0", "1", "4"]
...
CENTER = json.load(open(os.environ.get("FIT_CENTER", "/home/plaif/fit5_center.json")))


def sample(rng):
    # Second local refinement, now around round 4's best. The floor is known: an individual
    # deployment run sits W1 8.7 (left) / 9.3 (right) from the pooled deployment average, so no
    # single rendering can score below ~18 except by luck. Round 4 reached 22.2, so this round
    # keeps round 4's step sizes rather than tightening them -- there is little left to descend
    # into, and the remaining question is a randomisation range, not a point estimate.
    p = dict(CENTER)
